window.addEventListener("DOMContentLoaded", () => {
  ...
  if (featuredStoryWrap && newsGrid && newsEmptyState) {
    const storiesData = [
      {
        ...
        summary: "Our community garden initiative has brought together over 50 families, yielding fresh produce and stronger neighborhood bonds.",
        excerpt: "What started as a small pilot program has grown into a thriving community hub. Families gather weekly to tend their plots, share gardening tips, and build lasting friendships.",
      ...
      },
      {
        ...
        summary: "Our mindfulness and mental health workshops have helped hundreds of community members develop coping strategies and find support.",
        excerpt: "The latest round of sessions drew participants from across the region, showing the growing demand for accessible wellbeing education.",
        image: "https://images.unsplash.com/photo-1506126279646-a697353d3166?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        summary: "We’re excited to introduce a new mentorship program connecting local youth with caring adult role models.",
        excerpt: "The program focuses on confidence building, practical guidance, and long-term encouragement for young people in our community.",
        image: "https://images.unsplash.com/photo-1529390079861-591de354faf5?auto=format&fit=crop&w=900&q=80"
      },
      {
        id: "marias-journey",
        ...
        summary: "A community member shares how Harmony House services helped her navigate a difficult period and rebuild her life.",
        excerpt: "With support from counselling, community groups, and new friendships, Maria found renewed stability and confidence.",
        image: "https://images.unsplash.com/photo-1516302752625-fcc3c50ae61f?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        date: "March 10, 2026",
        author: "Emily Waters",
        summary: "Our chair yoga and gentle fitness classes for seniors now run four times weekly to accommodate growing interest.",
        excerpt: "Participants have reported improved mobility, confidence, and social connection through the expanded programme.",
        image: "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        date: "March 5, 2026",
        author: "Harmony House Team",
        summary: "We marked our first anniversary with a community celebration, reflecting on a year of growth, connection, and positive impact.",
        excerpt: "Residents, volunteers, and partners joined together to celebrate stories of resilience and success.",
        image: "https://images.unsplash.com/photo-1511632765486-a01980e01a18?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        author: "Harmony House Team",
        summary: "Thanks to a new partnership, community members can now access free legal consultation services twice monthly.",
        excerpt: "This new offering broadens the practical support available through the hub and improves local access to advice.",
        image: "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        author: "James Li",
        summary: "Our weekly teen support circles are creating a judgement-free space for growth, friendship, and resilience.",
        excerpt: "Young people say the group has become a trusted place to connect, share experiences, and feel understood.",
        image: "https://images.unsplash.com/photo-1529390079861-591de354faf5?auto=format&fit=crop&w=900&q=80"
      },
      {
        ...
        author: "Events Team",
        summary: "Our spring lineup features exciting new workshops, fitness classes, and community activities for all ages.",
        excerpt: "The new calendar reflects member feedback and includes several expanded programs requested by the community.",
        image: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&w=900&q=80"
      }
    ];
    ...
    function saveFavouriteStories() {
      localStorage.setItem("harmonyLikedStories", JSON.stringify([...favouriteStories]));
    }
    ...
    function renderFeaturedStory() {
      ...
      featuredStoryWrap.innerHTML = `
        <article class="featured-story-card">
          <button
            class="featured-story-like-btn ${favouriteStories.has(featured.id) ? "active" : ""}"
            type="button"
            data-story-id="${featured.id}"
            aria-label="Toggle liked story"
          >
            ♥
          </button>
    ...
    function createStoryCard(story) {
      ...
      article.innerHTML = `
        <div class="news-card-media">
          ...
          <button
            class="news-card-like-btn ${favouriteStories.has(story.id) ? "active" : ""}"
            type="button"
        ...
        <div class="news-card-body">
          ...
          <h3><a href="story.html?id=${encodeURIComponent(story.id)}">${story.title}</a></h3>
          <p class="news-card-summary">${story.summary}</p>
          <div class="news-card-footer">
            <span class="news-card-author">✎ ${story.author}</span>
            <a class="news-read-more" href="story.html?id=${encodeURIComponent(story.id)}">Read More →</a>
          </div>
        </div>
      `;
    ...
    function getVisibleStories() {
      ...
      return nonFeaturedStories.filter((story) => story.category === selectedStoryCategory);
    }
    ...
    function syncStoryButtons(storyId) {
      document.querySelectorAll(`[data-story-id="${storyId}"]`).forEach((btn) => {
        btn.classList.toggle("active", favouriteStories.has(storyId));
      });
    }

    function handleStoryLikeClick(event) {
      const button = event.target.closest(".news-card-like-btn, .featured-story-like-btn");
      if (!button) return;
      ...
      syncStoryButtons(storyId);
    }

    featuredStoryWrap.addEventListener("click", handleStoryLikeClick);
    newsGrid.addEventListener("click", handleStoryLikeClick);
    ...
    if (newsletterForm && newsletterEmail && newsletterMessage) {
      newsletterForm.addEventListener("submit", (event) => {
        ...
        newsletterMessage.hidden = false;
        newsletterMessage.textContent = "Thanks for subscribing! A confirmation email will be sent shortly.";
  ...
  if (contactForm && contactTopic) {
    ...
    const currentUser = JSON.parse(localStorage.getItem("harmonyCurrentUser") || "null");
  ...
  if (
    ...
    function renderDashboard() {
      ...
      const likedStoryMap = {
        ...
        "senior-fitness-classes-expand": {
          title: "Senior Fitness Classes Expand Due to Popular Demand",
          subtitle: "Program Update"
        },
        "community-celebrates-anniversary": {
          title: "Community Celebrates One Year of Harmony House",
          subtitle: "Milestone"
        },
      ...
      if (likedStories.length) {
        ...
        likedStories.forEach((storyId) => {
          const storyInfo = likedStoryMap[storyId];
          if (storyInfo) {
            accountLikedStoriesList.appendChild(
              createMiniCard(storyInfo.title, storyInfo.subtitle)
            );
          }
        });

        if (!accountLikedStoriesList.children.length) {
